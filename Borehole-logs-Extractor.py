...
def get_string_list(
    ...
    if target_string in text_content:  # 包含柱状图标题所有字符的文字对象
...
def get_neraby_text(
    ...
    if "@" in navigation:
        ...
        part_str = navigation.split("@")[1]  # 目标文字中要求包含某字符串
        navigation = temp
    ...
    if navigation != "标题":  # 找标题附近文字对象不依赖四周横竖线
    ...
    if navigation == "上":
        ...
        try:
            ...
            #           ' 上方文字对象内容为：空，请检查对应目标项所填方向是否错误【改成"下"？"右"？】\n')
            return
        if len(target_text_list) == 0:
            ...
            content = up_content
            # print('柱状图标识：', range_id, '字段名：' + field_name + ' 上方文字对象内容为:【',
            #       up_content + '】'+ print_add )
            # txt.write('柱状图标识：' + str(range_id) + ' 字段名：' + field_name +
            #           ' 上方文字对象内容为:【' + str(up_content) + '】'+ print_add +'\n')
            # for index, target, in enumerate(target_text_list):
            # print('柱状图标识：', range_id, '字段名：', field_name, '序号：', index,
            #       '右侧文字对象内容为：', target[1], '坐标为', target[2])
    ...
    if navigation == "右":
        ...
        try:
            ...
            # txt.write('柱状图标识：' + str(range_id) + ' 字段名：' + field_name +
            #           ' 右侧文字对象内容为：空，请检查对应目标项所填方向是否错误【改成"下"？】\n')
            return
        if len(target_text_list) == 0:
            pass
            ...
            # print('柱状图标识：', range_id, '字段名：' + field_name + ' 右侧文字对象内容为:【',
            #       right_content + '】' + print_add)
            # txt.write('柱状图标识：' + str(range_id) + ' 字段名：' + field_name +
            #           ' 右侧文字对象内容为:【' + str(right_content) + '】' + print_add +
            #           '\n')
            # for index, target, in enumerate(target_text_list):
            # print('柱状图标识：', range_id, '字段名：', field_name, '序号：', index,
            #       '右侧文字对象内容为：', target[1], '坐标为', target[2])
    ...
    if navigation == "下":
        ...
        if len(target_text_list) == 0:
            ...
            # print('柱状图标识：', range_id, '字段名：', field_name, ' 下方文字对象内容为:【',
            #       down_content + '】' + print_add)
            # txt.write('柱状图标识：' + str(range_id) + ' 字段名：' + field_name +
            #           ' 下方文字对象内容为:【' + str(down_content) + '】' + print_add +
            #           '\n')
   ...
   # 添加一个按钮，用于切换控制台的显示和隐藏
    ...
    def go(self):
        try:
            ...
            )  # max_frame_list,vertical_line_list和vertical_polyline_list里面的坐标x相应改动，有些坑爹的图文字对象插入点在外框外面，可以给外框适当加宽一点
            ...
            use_insertion_point = common_setting_dict[
                "插入点或中心点"
            ]  # 文字对象(含块参照名称)用插入点坐标(填 1 ),默认用中心点( 填 0 )
            ...
            #     "=柱":"取样标贯符号",
            #     "力学数据":""
            # }  #层底深度等多个文字对象参考字典:{CAD中的字段名，目标字段名}（CAD中的字段名分块会自动连接）字典键不要有空格
            # target_text_dict = {
            #     # "审  核": "下",
            #     "工程名称": "右",
            #     "工点名称": "右",
            ...
            if use_insertion_point == 1:
                print("文字对象(含块参照)用【插入点】坐标")
            else:
                print("文字对象(含块参照)用【中心点】坐标")
            print("--------------------------------------------")
            ...
            if (
                ...
                if extend_width != 0:
                    print(
                        "注意！为了读取部分插入点在框外的文字对象，现在外框左右分别增加了宽度："
                        + str(extend_width),
                        "\n如柱状图文字对象插入点均在外框内，为了更准确的处理，应将变量extend_width值改为0！",
                    )
                if extend_bottom_height != 0:
                    print(
                        "注意！为了读取部分插入点在框外的文字对象，现在外框底部分别增加了高度："
                        + str(extend_bottom_height),
                        "\n这样可能导致底部审核、编制等内容插入岩土描述中！如柱状图文字对象插入点均在外框内，为了更准确的处理，应将变量 extend_bottom_height 值改为0！",
                    )
                print("^^^^^^^^^^^^^^^^^^^^")
                ...
                for text in acad.iter_objects("AcDbText"):
                    ...
                    text_content = text.TextString  # 文字对象内容
                    text_insert_coordinate = text.InsertionPoint  # 文字对象插入点坐标
                    if (
            ...
            print(
                str(current_times)
                + "  正在遍历块参照对象，将会把块参照对象的名称合并到文字对象列表中一并处理..."
            )
            # 遍历块参照对象，以对象名称为内容，将块参照合并到下面的文字对象列表中
            BlockReference_list = []
            ...
            #############################################
            current_times = get_current_time()
            print(str(current_times) + "  正在遍历文字对象(包括多行文字对象MText)...")
            # 遍历cad文字对象，寻找目标字符串对象
            for text in acad.iter_objects(
                ["AcDbText", "AcDbMText"]
            ):  # ,'AcDbMText']):  #遍历'Text'类型对象并输出内容和坐标
                text_content = text.TextString.strip()  # 文字对象内容
            ...
            ########################################################
            text_list = text_list + BlockReference_list  # 将块参照名称列表并入文字对象列表
            # for text in text_list:
            #     print(text)
            #########################################################
            ...
            for id in [id[0] for id in max_frame_list]:
                ...
                ##A.钻孔信息表内容识别
                # 1、cad中的文字对象是否在输入的目标字段列表中（字符串精确匹配）
                drill_hole_info_list = []
                match_target_text_keys_list = []#填写的字段名称与cad中的文字对象已匹配上列表
                ...
                print('------------------------------------------------------')
                print('填写的字段名称与cad中的文字对象匹配不上列表:')
                for key in unique_to_no_space_target_text_dict_keys_list:
                    print(key)

                # 2、精确匹配不上的目标字段，把它拆开成单个字的列表，用每个字单独与cad中的文字对象做匹配
                ...
                for key in unique_to_no_space_target_text_dict_keys_list:
                    ...
                    for char in single_char_list:
                        print('待匹配单个字：',char)
                    print('------------------------------------------------------')
                    maybe_in_the_same_cell_list = []#潜在的可能是目标字段的单个文字对象列表
                    for text in text_with_target_id_list:
                        text_range_id = text[0]
                        text_content = text[1].strip()
                        # print(text_content)
                    ...
                    print('cad中潜在待合并字段单个文字对象列表',result_char_list)
                ...
                # 3、还是匹配不上的目标字段（key），进行局部匹配，看它是否完全包含于cad中的某个文字对象
                print('------------------------------------------------------')
                print('排除新字段前填写的字段名称与cad中的文字对象匹配不上列表:')
                for key in unique_to_no_space_target_text_dict_keys_list:
                    print(key)
                print('------------------------------------------------------')
                print('排除新字段后填写的字段名称与cad中的文字对象匹配不上列表:')
                ...
                for key in unique_to_no_space_target_text_dict_keys_list:
                    ...
                    for text in text_with_target_id_list:
                        ...
                        if key in text_content.replace(" ", ""):
                            ...
                            if target_value != None:
                                print('------------------------------------------------------')
                                print('目标字段与cad中文字对象进行局部匹配后，匹配到的cad中的字段和cad中的目标属性：',text_content,target_value,'目标字段是：',key)
                ...
                for key_string in no_space_multi_target_text_dict_keys_list:
                    ...
                    key_cell_list = list(
                        set(key_cell_list)
                    )  # 删除重复项(同一个字段在cad中可能分成几个文字对象，例如'层底','深度','m'实为一个字段名，位于同一个格子中)
                ...
                for field in target_key_range_list:
                    ...
                    if field[1] == "层底深度":  # 从层底深度开始搜索
                        ...
                        YT_MText_list = []  # 多行文字列表
                        target_value_list = []
                        i = 0

                        # time_reason_range
                        # 时代成因下方文字对象列表(绑定bounding_box有交集的对象)
                        if use_insertion_point != 1:
                            time_reason_list = [
                                time_reason
                        ...
                        #     print(k)
                        # exit()
                        # 剖面层号下方文字对象列表(绑定bounding_box有交集的对象)
                        if use_insertion_point != 1:
                            PM_list = [
                                PM_order
                        ...
                        for depth in depth_list:
                            ...
                            text_insertion_point_x = depth[4]  # 深度文字对象插入点x
                            text_insertion_point_y = depth[5]  # 深度文字对象插入点y
                        ...
                        if len(YT_MText_list) == 0:
                            print("【CAD文件中的岩土描述用文字(单行)表达】")
                            txt.write("【CAD文件中的岩土描述用文字(单行)表达】" + "\n")
                            print("")
                            ...
                            if len(sort_obj_id_YT_maybe_wrong_list) != 0:
                                ...
                                for ytms in sorting_ytms_list:
                                    ...
                                    ytms_content = ytms[1]  # 岩土描述文字对象内容
                                ...
                                print("【CAD文件中的岩土描述用多行文字表达】")
                                txt.write("【CAD文件中的岩土描述用多行文字表达】" + "\n")
                                print("")
                                ytms_list = (
                                    YT_MText_list  # 多行文字的岩土描述列表(层号，描述内容)
                                )
                            ####################################################################################
                            ####################################################################################
                            ####################################################################################
                ...
                #     for item in GG_list:
                #         print(item)
                TG_BG_list = new_TG_BG_list  # 同行文字合并后的结果列表
                # exit()
                ...
                if single_or_multiple_column == 0:  # 单列(多为分数形式)
                    ...
                    for item in TG_BG_list:
                        ...
                        content_list = item[4]  # 对应字段名下方格子内的文字对象列表
                        content_list = sorted(
                        ...
                        if text_obj_count % 2 == 0:
                            ...
                            print(
                                key_repr_field_name,
                                "：某些分数不完整，判断不了了，自己打吧(分数可能不是仅由两个文字对象组成)",
                            )
                            if extend_bottom_height != 0:
                                print(
            ...
            if use_insertion_point == 1:
                print("文字对象(含块参照)用【插入点】坐标")
                txt.write("文字对象(含块参照)用【插入点】坐标" + "\n")
            else:
                print("文字对象(含块参照)用【中心点】坐标")
                txt.write("文字对象(含块参照)用【中心点】坐标" + "\n")
            print("--------------------------------------------")
            print(
                "柱状图数量："
                + str(max_frame_count)
